...
func main() {
	...
	backend := echo.New()
	backend.Use(middleware.CORSWithConfig(middleware.CORSConfig{AllowOrigins: []string{"http://localhost:22222", "http://localhost:3000", "https://jwgoserver.azurewebsites.net", "https://jwreactclient.azurewebsites.net"}, AllowMethods: []string{http.MethodGet, http.MethodPost}}))
	database := connectDB()
